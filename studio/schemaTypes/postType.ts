...
export const postType = defineType({
  ...
  fields: [
    ...
    // ── Body ───────────────────────────────────────────────────
    // Portable Text with heading, image, code and link annotations.
    // This replaces the old Hygraph "sections[]" structure with a
    // single rich-text field — much simpler to author.
    defineField({
      name: 'body',
      title: 'Body',
      type: 'array',
      of: [
        ...
        // ── Code blocks ──
        // Renders as the codeblock_root__pf0C4 container (graph-slider UI)
        defineArrayMember({
          type: 'object',
          name: 'codeBlock',
          title: 'Code block',
    ...
    // Renders as the numbered <ol> section matching index reference.html
    defineField({
